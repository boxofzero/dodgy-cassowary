...
export const useStaminaStore = defineStore('stamina', () => {
  ...
  function syncStaminaData() {
    ...
    const additionalStamina = Math.floor(diffTime / (secondsPerStamina.value * 1000))
  ...
  function updateStaminaOverflow(additionalStamina: number) {
    updateStamina(additionalStamina, true)
  }

  function updateStamina(additionalStamina: number, allowOverflow = false) {
    if (!allowOverflow) {
